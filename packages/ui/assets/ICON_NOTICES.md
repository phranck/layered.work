...
The five SVGs in `brands/` are unmodified files from Simple Icons 15.16.0:
...
1. Confirm that the social account is configured (the original list is `SOCIAL_ACCOUNTS` in the prototype) and check the brand owner's current usage rules.
...
4. Add the slug to `BrandName` in `packages/ui/src/brand-mark.tsx` and its static mask rule in `brand-mark.css`.
5. Add the exact filename and byte comparison to `apps/dashboard/build.test.mjs`, then run the dashboard build test.
